...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // La acción de borrado se procesa primero (payload POST separado).
        ...
        if ($deleteEpisodeId > 0) {
            ...
            if ($deleteStmt->rowCount() > 0) {
                ...
                if ($editingEpisodeId === $deleteEpisodeId) {
                    $editingEpisodeId = null;
                    $isEditing = false;
                    $form = [
                        'guid' => '',
    ...
    if (stripos($message, 'UNIQUE constraint failed: episodes.guid') !== false) {
        ...
        echo 'Error en episodes_management.php: ' . $message . "\n";
        exit;
    }
}
?>
...
<body>
  <div class="container">
    <main class="card">
      ...
      <form method="post" action="episodes_management.php<?= $isEditing && $editingEpisodeId !== null ? '?episode_id=' . (int) $editingEpisodeId : '' ?>" autocomplete="off" enctype="multipart/form-data">
              ...
              <tr>
                ...
                <td>
                  <div class="row-actions">
                    <a class="edit-link" href="episodes_management.php?episode_id=<?= (int) ($episode['id'] ?? 0) ?>">Editar</a>
                    <form class="inline-form" method="post" action="episodes_management.php" onsubmit="return confirm('Se borrará el capítulo de la base de datos. ¿Continuar?');">
